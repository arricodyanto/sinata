...
import data3 from '@/json/tb_laylivestr.json'
...
export default function TableManajemenLiveStreaming() {
    // Table Data props
    const headers = [
      'ID', 'Judul Kegiatan', 'User Pemohon', 'Tanggal Kegiatan', 'Waktu', 'Tempat', 'Disposisi', 'Luaran Layanan', 'Aksi', 'Status'
    ]
    const columns = [
      { id: 1, label: 'id' },
                    ...
                    { rows.filter((item:any) => item.id === currIndex).map((data:any) => {
                        return(
                            <>
                                ...
                                { thumbnail == false && data.thumbnail_kegiatan != null ? (
                                      <>
                                          <FormLabel className='mb-2 text-sm'>Thumbnail Video Youtube</FormLabel>
                                          <Stack direction='row' spacing={1} justifyContent='space-between' alignItems='center' className='mb-4'>
                                              <Link href='/' target='_blank'>
                                                  <Typography className='text-sm hover:text-primary hover:underline hover:underline-offset-2 transition'>{data.thumbnail_kegiatan}</Typography>
                                              </Link>
                                              <Button size='small' disableElevation className='rounded-md capitalize py-1 px-3' onClick={() => setThumbnail(true)}>Change File</Button>
                                          </Stack>
                                      </>
                                  ) : (
                                      <>
                                          <FileUpload name='thumbnail_kegiatan' label='Thumbnail Video Youtube' allowMultiple={false} allowReorder={false} acceptedFileTypes={['application/pdf']} labelFileTypeNotAllowed='Hanya file PDF yang diijinkan' />
                                           { data.thumbnail_kegiatan != null ? (
                                              <Stack direction='row-reverse' className='-mt-2'>
                                                  <Button size='small' disableElevation className='rounded-md capitalize py-1 px-3' onClick={() => setThumbnail(false)}>Cancel</Button>
                                              </Stack>
                                           ) : null}
                                      </>
                                  )
