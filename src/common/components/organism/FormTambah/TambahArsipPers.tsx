...
import { dateISOFormatter, timeFormatter, timeISOFormatter } from '@/common/utils/dateFormatter.util';
    ...
    return (
        <>
            <CollapsibleAlert type='warning' className='mb-4'>
                <Typography className='text-dark' variant='body2'>Pastikan detail informasi kegiatan dan layanan peliputan telah ditambahkan pada sistem! Jika belum, silakan menuju ke halaman <Link href='/admins/daftar-kegiatan/tambah' className='underline hover:opacity-75 transition'>Tambah Kegiatan</Link> untuk menambahkan data kegiatan.</Typography>
            </CollapsibleAlert>
            <AutocompleteCustom name='judul_kegiatan' label='Judul Kegiatan' data={peliputans} getOptionLabel={(data) => data.tb_kegiatan.judul_kegiatan} onChange={handleJudulChange} />
            ...
            <TextfieldLabel name='no_rilis' label='No Rilis' value={no_rilis} onChange={(event: any) => setNo_rilis(event.target.value)} placeholder='xx/xxx/xxxxx' />
            <TextfieldLabel name='judul_berita' label='Judul Berita' value={judul_berita} onChange={(event: any) => setJudul_berita(event.target.value)} placeholder='Judul Berita' />
            <Stack direction='row' spacing={1}>
                <SelectLabel name='kategori' label='Kategori Berita' value={kategori} onChange={(event: any) => setKategori(event.target.value)} className='capitalize' >
                ...
                <AutocompleteCustom name='jurnalis' label='Jurnalis' data={users} getOptionLabel={(data) => data.name} onChange={handleJurnalisChange} />
            </Stack>
            <TextfieldLabel name='prarilis' label='Prarilis' value={prarilis} onChange={(event: any) => setPrarilis(event.target.value)} multiline maxRows={16} placeholder='Naskah Prarilis' />
            <TextfieldLabel name='rilis' label='Rilis' value={rilis} onChange={(event: any) => setRilis(event.target.value)} multiline maxRows={16} placeholder='Naskah Rilis Berita setelah Editing' />
            <Stack direction='row' spacing={1} className='mb-4'>
                <FormControl className='w-full'>
                    <FormLabel className='mb-1 text-sm'>
                        Tanggal Rilis
                    </FormLabel>
                    <DatePickerBasic onChange={(value: any) => setTgl_upload(dateISOFormatter(value))} />
                </FormControl>
                <FormControl className='w-full'>
                    <FormLabel className='mb-1 text-sm'>
                        Waktu Rilis
                    </FormLabel>
                    <TimePickerBasic onChange={(value: any) => setWaktu_upload(timeISOFormatter(value))} />
                </FormControl>
            </Stack>
            <Stack direction='row' spacing={1}>
                <AutocompleteCustom label='Admin' data={users} onChange={handleAdminChange} getOptionLabel={(data) => data.name} />
                <TextfieldLabel name='link_berita' label='Tautan Berita' value={link_berita} onChange={(event: any) => setLink_berita(event.target.value)} placeholder='Tautan Berita Terpublikasi' />
            </Stack>
            <TextfieldLabel name='judul_terjemahan' label='Judul Terjemahan (EN)' value={judul_terjemahan} onChange={(event: any) => setJudul_terjemahan(event.target.value)} placeholder='Judul Berita untuk Bahasa Inggris' />
            <AutocompleteCustom label='Penerjemah' data={users} onChange={handlePenerjemahChange} getOptionLabel={(data) => data.name} />
            <TextfieldLabel name='naskah_terj' label='Naskah Terjemahan' value={naskah_terj} onChange={(event: any) => setNaskah_terj(event.target.value)} multiline maxRows={16} placeholder='Naskah Terjemahan dari Rilis Berita' />
            <Stack direction='row' spacing={1} className='mb-4'>
                <FormControl className='w-full'>
                    <FormLabel className='mb-1 text-sm'>
                        Tanggal Rilis Terjemahan
                    </FormLabel>
                    <DatePickerBasic onChange={(value: any) => setTgl_upload_terj(dateISOFormatter(value))} />
                </FormControl>
                <FormControl className='w-full'>
                    <FormLabel className='mb-1 text-sm'>
                        Waktu Rilis Terjemahan
                    </FormLabel>
                    <TimePickerBasic onChange={(value: any) => setWaktu_upload_terj(timeFormatter(value))} />
                </FormControl>
            </Stack>
            <Stack direction='row' spacing={1}>
                <AutocompleteCustom label='Admin Terjemahan' data={users} onChange={handleAdminTerjChange} getOptionLabel={(data) => data.name} />
                <TextfieldLabel name='link_terj' label='Tautan Terjemahan' value={link_terj} onChange={(event: any) => setLink_terj(event.target.value)} placeholder='Tautan Berita untuk Bahasa Inggris' />
            </Stack>
            <SelectLabel name='status' label='Status Publikasi' value={status_publikasi} onChange={handleStatusPublikasiChange}>
                <MenuItem value='Pending'>Pending</MenuItem>
