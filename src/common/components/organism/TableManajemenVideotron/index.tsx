...
import { getAllLayananVideotron } from '@/services/layanan-videotron';
import { dateFormatter, dateStringFormatter, timeFormatter, timeStrictFormatter } from '@/common/utils/dateFormatter.util';
import Image from 'next/image';

export default function TableManajemenVideotron() {
    const { isReady, push } = useRouter();

    const headers = [
        'Judul Kegiatan', 'User Pemohon', 'Tanggal Kegiatan', 'Waktu', 'Tempat', 'Bahan Publikasi', 'Disposisi', 'Luaran Layanan', 'Aksi', 'Status'
    ];
    const columns = [
        ...
        { id: 6, label: 'bahan_publikasi' },
        { id: 7, label: 'disposisi' },
        { id: 8, label: 'luaran_layanan' }
    ];
    ...
    const getVideotron = useCallback(async () => {
        const params = `limit=${rowsPerPage}&page=${page}`;
        const response = await getAllLayananVideotron(params);
        setData(response.data);
        setTotalRow(response.totalRow);
        setRowsPerPage(response.rowsPerPage);
    }, [getAllLayananVideotron, page, rowsPerPage]);
            ...
            {data.length === 0 ?
                <>
                    <TableDataEmpty headers={headers}
                        addButton={
                            <Link href='/admins/riwayat-ajuan/Layanan Penayangan Konten di Videotron/tambah'>
                    ...
                    <Box className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-md xs:w-[calc(100%-40px)] md:w-[800px]'>
                        <Stack direction='row' justifyContent='space-between' className='sticky py-2 px-6 bg-gray-100 rounded-t-md'>
                            <Typography variant="subtitle1" component="h2" className='font-bold'>
                                Manajemen Layanan Penayangan Konten di Videotron UNS
                            </Typography>
                                    ...
                                    <>
                                        ...
                                        <Stack direction='row' spacing={1} className='mb-6'>
                                            <FormControl className='w-full'>
                                                ...
                                                <DateFieldBasic name='tgl_kegiatan' defaultValue={dayjs(dateFormatter(item.tgl_awal), 'DD/MM/YYYY')} />
                                            </FormControl>
                                            <FormControl className='w-full'>
                                                <FormLabel className='mb-1 text-sm'>
                                                    Tanggal Akhir Penayangan
                                                </FormLabel>
                                                <DateFieldBasic name='tgl_kegiatan' defaultValue={dayjs(dateFormatter(item.tgl_akhir), 'DD/MM/YYYY')} />
                                            </FormControl>
                                        </Stack>
                                        ...
                                        <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems='flex-start' className='mb-4'>
                                            {item.luaran_layanan ? (
                                                <Link href={`${api_file}/${item.luaran_layanan}`} target='blank' className='w-[20rem] mt-2'>
                                                    <Image src={`${api_file}/${item.luaran_layanan}`} alt={`${item.tb_kegiatan.judul_kegiatan}`} quality={80} layout='responsive' width={20} height={20} className='rounded-lg' />
                                                </Link>
                                            ) : (
                                                <Typography variant='body2' className='italic'>Belum ada data.</Typography>
                                            )}
                        ...
                        <Stack direction='row' justifyContent='flex-end' spacing={1} margin={2} marginBottom={1}>
                            ...
                            <ButtonIcon variant='outlined' icon={<ArrowForwardIcon className='-mr-1' />} onClick={() => push(`/admins/riwayat-ajuan/Layanan Penayangan Konten di Videotron/${currIndex}`)}>Lihat Ajuan</ButtonIcon>
                        </Stack>
                    </Box>
                </Fade>
            </Modal>
