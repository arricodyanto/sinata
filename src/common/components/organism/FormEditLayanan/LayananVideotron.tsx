...
import { deleteOneLayananVideotron, updateLayananVideotron } from '@/services/layanan-videotron';
...
export default function LayananVideotron(props: TFormEditLayananProps) {
    ...
    const onSave = async () => {
        ...
        if (isSame === false) {
            const response = await updateLayananVideotron(id, form);
            ...
            if (response.error === false) {
                ...
                isAdmin ? push('/admins/layanan-media') : push('/users/profile');
            }
        }
        setOpenSimpan(false);
    };
    ...
    return (
        <>
            <Typography variant='h5' className='mb-6'>Layanan Penayangan Konten di Videotron</Typography>
            {rows.map((data: any) => {
                return (
                    <>
