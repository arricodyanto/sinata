...
import data3 from '@/json/tb_laybaliho.json';
...
export default function TableManajemenBaliho() {
    // Table Data props
    const headers = [
        'ID', 'Judul Kegiatan', 'User Pemohon', 'Tanggal Kegiatan', 'Waktu', 'Tempat', 'Bahan Publikasi', 'Tanggal Mulai Penayangan', 'Tanggal Akhir Penayangan', 'Disposisi', 'Luaran Layanan', 'Aksi', 'Status'
    ];
    const columns = [
        { id: 1, label: 'id' },
        ...
        { id: 8, label: 'tgl_awal' },
        { id: 9, label: 'tgl_akhir' },
        { id: 10, label: 'disposisi' },
        { id: 11, label: 'luaran_layanan' },
    ];
    ...
    const [buktiPembayaran, setBuktiPembayaran] = React.useState(false);
                    ...
                    <Box className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-md xs:w-[calc(100%-40px)] md:w-[800px]'>
                        <Stack direction='row' justifyContent='space-between' className='sticky py-2 px-6 bg-gray-100 rounded-t-md'>
                            <Typography variant="subtitle1" component="h2" className='font-bold'>
                                Manajemen Layanan Pemasangan Baliho
                            </Typography>
                                    ...
                                    <>
                                        ...
                                        <Stack direction='row' spacing={1} className='mb-6'>
                                            <FormControl className='w-full'>
                                                ...
                                                <DateFieldBasic name='tgl_kegiatan' defaultValue={dayjs(data.tgl_awal, 'DD/MM/YYYY')} />
                                            </FormControl>
                                            <FormControl className='w-full'>
                                                <FormLabel className='mb-1 text-sm'>
                                                    Tanggal Akhir Penayangan
                                                </FormLabel>
                                                <DateFieldBasic name='tgl_kegiatan' defaultValue={dayjs(data.tgl_akhir, 'DD/MM/YYYY')} />
                                            </FormControl>
                                        </Stack>
                                        {disposisi == false ? (
                                            <>
                                        ...
                                        {buktiPembayaran == false ? (
                                            <>
                                                ...
                                                <Stack direction='row' spacing={1} justifyContent='space-between' alignItems='center' className='mb-4'>
                                                    <Link href='/' target='_blank'>
                                                        <Typography className='text-sm hover:text-primary hover:underline hover:underline-offset-2 transition'>{data.bukti_pembayaran}</Typography>
                                                    </Link>
                                                    <Button size='small' disableElevation className='rounded-md capitalize py-1 px-3' onClick={() => setBuktiPembayaran(true)}>Change File</Button>
                                                </Stack>
                                            </>
                                        ) : (
                                            <>
                                                <FileUpload name='bukti_pembayaran' label='Bukti Pembayaran' allowMultiple={false} allowReorder={false} acceptedFileTypes={['application/pdf']} labelFileTypeNotAllowed='Hanya file PDF yang diijinkan' />
                                                <Stack direction='row-reverse' className='-mt-2 mb-4'>
                                                    <Button size='small' disableElevation className='rounded-md capitalize py-1 px-3' onClick={() => setBuktiPembayaran(false)}>Cancel</Button>
                                                </Stack>
                                            </>
                                        )
                                        }
