...
export default function SignUpForm() {
  ...
  return (
    <Box className='mt-9'>
      <TextfieldIcon label='Nama Pengguna' value={username} onChange={handleFormChange(setUsername)} icon={<AlternateEmailIcon />} placeholder='johndoe' className='mb-6' />
      <TextfieldIcon label='Nama Lengkap' value={name} onChange={handleFormChange(setName)} icon={<AccountCircleIcon />} placeholder='John Doe' className='mb-6' />
      <TextfieldIcon label='Alamat Email' value={email} onChange={handleFormChange(setEmail)} icon={<EmailIcon />} placeholder='johndoe@yourmail.com' className='mb-6' />
      ...
      <Button variant='contained' onClick={onSubmit} disabled={!checked} fullWidth color='primary' className='mt-4 mb-1'>Register</Button>
    </Box>
  );
}
