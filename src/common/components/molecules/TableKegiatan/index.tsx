...
import {
	deleteOneDataKegiatan,
	getAllDataKegiatan,
	getAllDataKegiatanUser,
	updateOneDataKegiatan,
...
export default function TableKegiatan(props: TTableKegiatanProps) {
	const { admin } = props;
	...
	const onSave = async () => {
		...
		if (!isSame) {
			...
			if (response.error === false) {
				...
				handleCancelEdit();
				{
					admin ? getDataKegiatan() : getDataKegiatanUser();
				}
			}
		}
		setOpen(false);
				...
				<TableDataEmpty
					...
					addButton={
						<Link
							href={
								admin
									? '/admins/daftar-kegiatan/tambah'
									: '/users/tambah-kegiatan'
							}>
				...
				<Fade in={open}>
					<Box className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-md xs:w-[calc(100%-40px)] md:w-[600px]'>
						...
						<Box
							...
							sx={{ mt: 2 }}
							className='max-h-[70vh] overflow-auto pb-4 px-6'>
							{data
									...
									return (
										<>
											<TextfieldLabel
												...
												defaultValue={data.judul_kegiatan}
												onChange={(event: any) =>
													form.set('judul_kegiatan', event.target.value)
												}
												disabled={!editable}
											/>
											<TextfieldLabel
												...
												defaultValue={data.des_kegiatan}
												onChange={(event: any) =>
													form.set('des_kegiatan', event.target.value)
												}
												multiline
												maxRows={8}
												disabled={!editable}
											...
											<SelectLabel
												...
												defaultValue={data.sifat_kegiatan}
												onChange={(event: any) =>
													form.set('sifat_kegiatan', event.target.value)
												}
												disabled={!editable}>
											...
											<Stack
												...
												<FormControl className='w-full'>
													...
													<DatePickerBasic
														defaultValue={dayjs(
															dateFormatter(data.tgl_kegiatan),
															'DD/MM/YYYY',
														)}
														onChange={(value: any) =>
												...
												<FormControl className='w-full'>
													...
													<TimePickerBasic
														defaultValue={dayjs(
															dateFormatter(data.tgl_kegiatan) +
																' ' +
																timeStrictFormatter(data.waktu_kegiatan),
															'DD/MM/YYYY hh:mm',
														)}
														onChange={(value: any) =>
															form.set(
																'waktu_kegiatan',
																timeISOFormatter(value),
															)
														}
														disabled={!editable}
													/>
											...
											<TextfieldLabel
												...
												defaultValue={data.tempat_kegiatan}
												onChange={(event: any) =>
													form.set('tempat_kegiatan', event.target.value)
												}
												disabled={!editable}
											/>
											{admin ? (
												...
												<>
													...
													<Stack
														...
														{data.surat_permohonan ? (
															<Link
																href={`${api_file}/${data.surat_permohonan}`}
																target='_blank'>
											...
											<Typography
												...
												Ditambahkan pada {dateStringFormatter(data.createdAt)}
											</Typography>
											<DialogConfirmation
												title='Hapus'
