...
export default function EventCardV2(props: TEventCardV2Props) {
    const {
        id, image, visibility, publisher, avatar, title, description, date, time, location
    } = props;
    return (
        <>
                ...
                <CardContent className="max-h-52 pb-0">
                    ...
                    <Stack direction='row' alignItems='center' spacing={2}>
                        <Stack direction='row'>
                            ...
                            <Typography variant='caption' className='pl-1 text-gray-500'>{dateFormatter(date)}</Typography>
                        </Stack>
                        <Stack direction='row'>
                            <AccessTimeIcon fontSize='inherit' sx={{ fontSize: 18 }} color='primary' />
                            <Typography variant='caption' className='pl-1 text-gray-500'>{timeStrictFormatter(time)} WIB</Typography>
                        </Stack>
                    </Stack>
