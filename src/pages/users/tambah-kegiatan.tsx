...
export default function TambahKegiatan() {
	...
	const handleTambah = async (form: any) => {
		...
		const isEmpty =
			Array.isArray(formattedForm) &&
			formattedForm.length === 0 &&
			JSON.stringify(formattedForm) === JSON.stringify(emptyForm);
		if (isEmpty) {
			toast.error(
				'Tidak ada data yang tambahkan. Mohon masukkan data dengan benar.',
				{
					theme: 'colored',
				},
			);
		...
		if (!isEmpty) {
			const isRequiredFilled =
				form.get('judul_kegiatan') &&
				form.get('surat_permohonan') &&
				form.get('sik') &&
				form.get('tgl_kegiatan') &&
				form.get('waktu_kegiatan')
					? true
					: false;
			if (isRequiredFilled) {
		...
		<>
			<Box className='bg-grey'>
				...
				<DashboardPanel listMenu={listMenuUser}>
					<HeaderBreadcrumbs
						pageHeader='Tambah Kegiatan'
						currentPage='Tambah Kegiatan'>
						<Link
							...
							<Paper className='shadow-md px-6 py-4'>
								<CollapsibleAlert
									...
									<Typography
										variant='body2'
										className='text-dark'>
										Sebelum mengajukan layanan, harap pastikan judul dan detail
										acara Anda sudah terdaftar pada sistem !
									</Typography>
								</CollapsibleAlert>
